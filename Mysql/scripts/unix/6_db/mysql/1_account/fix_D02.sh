...
if [[ $RC -eq 124 ]]; then
  ...
  if [[ $FAIL -eq 0 && $RC2 -eq 0 && -z "$AFTER" ]]; then
    ...
    ACTION_LOG="불필요 계정 ${COUNT}건을 ${ACTION_MODE} 처리했습니다."
    EVIDENCE="D-02 조치 후 불필요 계정 미검출"
  else
    ACTION_LOG="조치 일부 실패: 불필요 계정 처리 후 잔여 계정이 남아 있을 수 있습니다."
