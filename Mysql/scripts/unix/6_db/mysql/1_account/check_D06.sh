    ...
    if [[ $RC2 -ne 0 ]]; then
      ...
      while IFS=$'\t' read -r user host locked; do
        ...
        if { [[ -z "$user" ]] && [[ $ANON_BAD -eq 1 ]]; } || [[ -n "${BAD_COMMON[$user]:-}" ]]; then
          if run_mysql "DROP USER IF EXISTS '${esc_user}'@'${esc_host}';" >/dev/null; then
            DROP_CNT=$((DROP_CNT + 1))
          elif run_mysql "ALTER USER '${esc_user}'@'${esc_host}' ACCOUNT LOCK;" >/dev/null; then
            LOCK_CNT=$((LOCK_CNT + 1))
          else
            FAIL=1
          fi
          ...
          case "$host" in
            ...
            *)
              if run_mysql "ALTER USER '${esc_user}'@'${esc_host}' ACCOUNT LOCK;" >/dev/null; then
                LOCK_CNT=$((LOCK_CNT + 1))
              else
                FAIL=1
              fi
