...
# INSTALL COMPONENT는 이미 설치된 경우 실패 가능 -> 분리 처리
...
if [[ $RC1 -ne 0 || $RC2 -ne 0 || $RC3 -ne 0 || $RC4 -ne 0 || $RC5 -ne 0 || $RC6 -ne 0 ]]; then
  ...
  if [[ $RCV -eq 0 && -n "$VERIFY" ]]; then
    ...
    ACTION_LOG="비밀번호 복잡도/수명 정책을 적용했습니다."
    EVIDENCE="D-03 정책값 적용 완료"
  else
...
cat <<JSON
{
  ...
  "status":"$STATUS",
  "evidence":"$EVIDENCE",
  "guide":"validate_password 및 default_password_lifetime 정책 적용",
