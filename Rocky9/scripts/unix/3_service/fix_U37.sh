...
if [ -f "$CRONTAB_CMD" ]; then
    ...
    ACTION_LOG="$ACTION_LOG $CRONTAB_CMD 권한 내역: $AFTER로 변경되었습니다."
fi
...
if [ -f "$AT_CMD" ]; then
    ...
    ACTION_LOG="$ACTION_LOG $AT_CMD 권한 내역: $AFTER로 변경되었습니다."
fi
    ...
    if [ -d "$dir" ]; then
        for f in "$dir"/*; do
            if [ -f "$f" ]; then
                ...
                ACTION_LOG="$ACTION_LOG $f 권한 내역: $AFTER로 변경되었습니다."
            fi
        done
    fi
done
...
if [ -n "$ACTION_LOG" ]; then
    ACTION_LOG="crontab 및 at 서비스 관련 파일의 소유자를 root로 변경하고 권한을 적절히 설정했습니다."
else
    ACTION_LOG="crontab 및 at 관련 파일이 이미 적절한 권한으로 설정되어 있습니다."
fi
