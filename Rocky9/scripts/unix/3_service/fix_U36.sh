...
# [r-command 신뢰 파일] /etc/hosts.equiv, $HOME/.rhosts 정비
# 가이드: 사용 여부 점검 및 권한 600 이하 유지
TRUST_FILES=(/etc/hosts.equiv /root/.rhosts /home/*/.rhosts)
for trust_file in "${TRUST_FILES[@]}"; do
    [ -f "$trust_file" ] || continue

    chmod 600 "$trust_file" 2>/dev/null
    ACTION_LOG="$ACTION_LOG $trust_file 권한을 600으로 변경했습니다."

    if grep -Ev '^[[:space:]]*#|^[[:space:]]*$' "$trust_file" 2>/dev/null | grep -q '.'; then
        cp "$trust_file" "${trust_file}.bak_$(date +%Y%m%d_%H%M%S)" 2>/dev/null
        sed -Ei 's/^([[:space:]]*[^#[:space:]].*)$/#\\1/' "$trust_file"
        ACTION_LOG="$ACTION_LOG $trust_file의 신뢰 접속 허용 항목을 비활성화했습니다."
    fi
done
