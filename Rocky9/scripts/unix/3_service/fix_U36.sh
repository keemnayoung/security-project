...
# @Importance : 상
# @Title : r 계열 서비스 비활성화
# @Description : rsh, rlogin, rexec 등 r 계열 서비스를 비활성화
...
if [ -f "/etc/inetd.conf" ]; then
    ...
    if [ $INETD_MODIFIED -eq 1 ]; then
        ...
        ACTION_LOG="$ACTION_LOG /etc/inetd.conf r계열 서비스 주석처리 및 inetd 재시작;"
    fi
fi
...
AFTER_SETTING="r 계열 서비스 비활성화 완료"
[ -z "$ACTION_LOG" ] && ACTION_LOG="r 계열 서비스가 이미 비활성화 상태"
