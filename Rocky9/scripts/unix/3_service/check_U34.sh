...
if [ -f "/etc/inetd.conf" ]; then
    ...
    # 주석(#)으로 시작하지 않는 finger 라인 검색
    if grep -v "^#" "$TARGET_FILE" 2>/dev/null | grep -qE "^[[:space:]]*finger"; then
        FINGER_ACTIVE=1
        EVIDENCE="$EVIDENCE /etc/inetd.conf에 Finger 서비스 활성화;"
    fi
fi
...
if [ -f "/etc/xinetd.d/finger" ]; then
    ...
    if grep -qiE "disable\s*=\s*no" "$TARGET_FILE" 2>/dev/null; then
        FINGER_ACTIVE=1
        EVIDENCE="$EVIDENCE /etc/xinetd.d/finger에서 disable = no;"
    fi
fi
...
if [ $FINGER_ACTIVE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="Finger 서비스가 활성화되어 있음:$EVIDENCE"
else
    STATUS="PASS"
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "xinetd에서 finger 서비스를 disable=yes로 설정하거나, inetd.conf에서 finger 라인을 주석처리 후 서비스를 재시작하세요.",
