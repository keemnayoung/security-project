#!/bin/bash
# ============================================================================
# @Project: 시스템 보안 자동화 프로젝트
# @Version: 1.0.0
# @Author: 이가영
# @Last Updated: 2026-02-06
# ============================================================================
# [점검 항목 상세]
# @Check_ID : U-50
# @Category : 서비스 관리
# @Platform : Rocky Linux
# @Importance : 상
# @Title : DNS Zone Transfer 설정
# @Description : Secondary Name Server로만 Zone 정보 전송 제한 여부 점검
# @Criteria_Good : DNS 서비스를 사용하지 않는 경우 서비스 중지 및 비활성화 설정
# @Criteria_Bad :  DNS 서비스 사용 시 DNS Zone Transfer를 허가된 사용자에게만 전송 허용하도록 설정
# @Reference : 2026 KISA 주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드
# ============================================================================

# [진단] U-50 DNS Zone Transfer 설정

# 1. 항목 정보 정의
ID="U-50"
CATEGORY="서비스 관리"
TITLE="DNS Zone Transfer 설정"
IMPORTANCE="상"
TARGET_FILE="/etc/bind/named.conf.options"

# 2. 진단 로직 (무결성 해시 포함)
STATUS="PASS"
EVIDENCE=""
FILE_HASH="NOT_FOUND"

VULNERABLE=0

# DNS 서비스(named) 실행 여부 확인
# 가이드: systemctl list-units --type=service | grep named
if ! systemctl list-units --type=service 2>/dev/null | grep -q named && ! pgrep -x named >/dev/null 2>&1; then
    STATUS="PASS"
    EVIDENCE="DNS 서비스가 비활성화되어 있어 점검 대상이 없습니다."
else
    # 설정 파일 찾기 (데비안/우분투: named.conf.options, RHEL: named.conf)
    # 가이드: /etc/named.boot, /etc/bind/named.boot, /etc/named.conf, /etc/bind/named.conf.options
    CONF_FILES=("/etc/named.boot" "/etc/bind/named.boot" "/etc/named.conf" "/etc/bind/named.conf.options" "/etc/bind/named.conf")
    FOUND_CONF=0
    
    # Include 파일 목록을 담을 배열
    INCLUDE_FILES=()
    
    for conf in "${CONF_FILES[@]}"; do
        if [ -f "$conf" ]; then
            FOUND_CONF=1
            TARGET_FILE="$conf"
            FILE_HASH=$(sha256sum "$TARGET_FILE" 2>/dev/null | awk '{print $1}')
            
            # Include 구문으로 참조하는 파일 찾기
            # 가이드: DNS 설정 파일의 Include 구문으로 참조하는 파일명 점검
            while IFS= read -r inc_file; do
                if [ -f "$inc_file" ]; then
                    INCLUDE_FILES+=("$inc_file")
                fi
            done < <(grep -hE "^[[:space:]]*include" "$conf" 2>/dev/null | sed 's/.*"\(.*\)".*/\1/')
            
            # 1. xfrnets 확인 (BIND 4/8 - 구형)
            # 가이드: cat /etc/named.boot | grep xfrnets
            if grep -q "xfrnets" "$conf"; then
                SETTING=$(grep "xfrnets" "$conf")
                EVIDENCE="$EVIDENCE $conf: $SETTING."
            fi
            
            # 2. allow-transfer 확인 (BIND 9)
            # 가이드: cat /etc/named.conf | grep allow-transfer
            if grep -q "allow-transfer" "$conf"; then
                SETTING=$(grep "allow-transfer" "$conf" | sed 's/^[ \t]*//')
                if echo "$SETTING" | grep -q "any"; then
                    VULNERABLE=1
                    EVIDENCE="$EVIDENCE $conf: allow-transfer { any; } 설정이 발견되었습니다."
                else
                    EVIDENCE="$EVIDENCE $conf: $SETTING (제한 설정되어 있습니다)."
                fi
            else
                # allow-transfer 설정이 없으면 기본적으로 allow (BIND 9 기준)
                VULNERABLE=1
                EVIDENCE="$EVIDENCE $conf: allow-transfer 설정이 없습니다(기본값: 모든 호스트 허용)."
            fi
        fi
    done
    
    # Include 파일들도 점검
    for inc_file in "${INCLUDE_FILES[@]}"; do
        EVIDENCE="$EVIDENCE Include 파일: $inc_file."
        
        # xfrnets 확인
        if grep -q "xfrnets" "$inc_file" 2>/dev/null; then
            SETTING=$(grep "xfrnets" "$inc_file")
            EVIDENCE="$EVIDENCE $inc_file: $SETTING."
        fi
        
        # allow-transfer 확인
        if grep -q "allow-transfer" "$inc_file" 2>/dev/null; then
            SETTING=$(grep "allow-transfer" "$inc_file" | sed 's/^[ \t]*//')
            if echo "$SETTING" | grep -q "any"; then
                VULNERABLE=1
                EVIDENCE="$EVIDENCE $inc_file: allow-transfer { any; }가 발견되었습니다."
            else
                EVIDENCE="$EVIDENCE $inc_file: $SETTING."
            fi
        fi
    done
    
    if [ $FOUND_CONF -eq 0 ]; then
        STATUS="PASS"
        EVIDENCE="DNS 설정 파일을 찾을 수 없습니다."
    elif [ $VULNERABLE -eq 1 ]; then
        STATUS="FAIL"
        EVIDENCE="DNS Zone Transfer 제한이 미흡하여, 비인가 호스트에서 DNS 영역 정보를 탈취할 수 있는 위험이 있습니다. $EVIDENCE"
    else
        STATUS="PASS"
        EVIDENCE="DNS Zone Transfer가 제한되어 있습니다: $EVIDENCE"
    fi
fi


IMPACT_LEVEL="MEDIUM"
ACTION_IMPACT="Zone Transfer 설정에서 허용할 대상을 정상적으로 등록하였다면 일반적으로 영향이 없으나, 허용 대상 등록을 정확히 반영하지 않으면 운영에 영향을 줄 수 있으므로 허용 대상(Secondary Name Server 등)을 사전에 점검·검증한 뒤 적용해야 합니다."

# 3. 마스터 템플릿 표준 출력
echo ""
cat << EOF
{
    "check_id": "$ID",
    "category": "$CATEGORY",
    "title": "$TITLE",
    "importance": "$IMPORTANCE",
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "named.conf에서 allow-transfer { none; }; 또는 신뢰된 Secondary DNS IP만 허용하도록 설정해야 합니다.",
    "target_file": "$TARGET_FILE",
    "file_hash": "$FILE_HASH",
    "impact_level": "$IMPACT_LEVEL",
    "action_impact": "$ACTION_IMPACT",
    "check_date": "$(date '+%Y-%m-%d %H:%M:%S')"
}
EOF
