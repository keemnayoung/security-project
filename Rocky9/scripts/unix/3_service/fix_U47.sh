...
if command -v sendmail &>/dev/null; then
    ...
    BEFORE_SETTING="$BEFORE_SETTING Sendmail 버전: $SENDMAIL_VER;"
    ...
    if [ -n "$MAJOR_VER" ] && [ -n "$MINOR_VER" ]; then
        if [ "$MAJOR_VER" -gt 8 ] || ([ "$MAJOR_VER" -eq 8 ] && [ "$MINOR_VER" -ge 9 ]); then
            ...
            if [ -f "$SENDMAIL_CF" ]; then
                if ! grep -q "Relaying denied" "$SENDMAIL_CF"; then
                    ...
                    ACTION_LOG="$ACTION_LOG sendmail.cf에 Relaying denied 룰 추가 (8.9 미만);"
                fi
            fi
        fi
    else
        ...
        makemap hash /etc/mail/access.db < /etc/mail/access 2>/dev/null
        ACTION_LOG="$ACTION_LOG access 파일 설정 (버전 확인 실패);"
    fi
    
    # 서비스 재시작
    systemctl restart sendmail 2>/dev/null
    ACTION_LOG="$ACTION_LOG Sendmail 재시작;"
fi
...
if command -v postfix &>/dev/null; then
    ...
    if [ -f "$MAIN_CF" ]; then
        if grep -q "mynetworks" "$MAIN_CF"; then
             BEFORE_SETTING="$BEFORE_SETTING Postfix mynetworks 설정 있음;"
             # 여기서는 자동 수정보다는 경고 및 로컬호스트만 허용 예시 주석 추가 정도가 안전
             # 가이드는 '허용할 네트워크 주소'를 넣으라고 함 -> 로컬호스트로 가정
        else
             BEFORE_SETTING="$BEFORE_SETTING Postfix mynetworks 없음;"
             echo "mynetworks = 127.0.0.0/8" >> "$MAIN_CF"
             ACTION_LOG="$ACTION_LOG Postfix mynetworks = 127.0.0.0/8 추가;"
        fi
        ...
        if ! grep -q "smtpd_recipient_restrictions" "$MAIN_CF"; then
            ...
            ACTION_LOG="$ACTION_LOG Postfix smtpd_recipient_restrictions 추가;"
        fi
        
        postfix reload 2>/dev/null
        ACTION_LOG="$ACTION_LOG Postfix reload;"
    fi
fi
...
if command -v exim &>/dev/null || command -v exim4 &>/dev/null; then
    ...
    for conf in "${CONF_FILES[@]}"; do
        if [ -f "$conf" ]; then
            ...
            if grep -v "^#" "$conf" | grep -qE "relay_from_hosts.*\*|relay_from_hosts.*0\.0\.0\.0/0"; then
                BEFORE_SETTING="$BEFORE_SETTING Exim relay_from_hosts에 모든 호스트 허용;"
                ...
                ACTION_LOG="$ACTION_LOG Exim relay_from_hosts를 127.0.0.1로 변경;"
                EXIM_MODIFIED=1
            fi
            ...
            if ! grep -q "relay_from_hosts" "$conf"; then
                ...
                ACTION_LOG="$ACTION_LOG Exim relay_from_hosts = 127.0.0.1 추가;"
                EXIM_MODIFIED=1
            fi
    ...
    if [ $EXIM_MODIFIED -eq 1 ]; then
        ...
        ACTION_LOG="$ACTION_LOG Exim 재시작;"
    else
        ACTION_LOG="$ACTION_LOG Exim 설정 이상 없음 또는 수동 확인 필요;"
    fi
fi

AFTER_SETTING="릴레이 제한 설정 조치 완료 (Sendmail/Postfix)"
[ -z "$ACTION_LOG" ] && ACTION_LOG="이미 설정되어 있거나 특이사항 없음"
