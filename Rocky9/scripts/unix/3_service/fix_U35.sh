...
if grep -q "^ftp:" /etc/passwd 2>/dev/null; then
    ...
    ACTION_LOG="$ACTION_LOG ftp 계정을 삭제했습니다."
fi
if grep -q "^anonymous:" /etc/passwd 2>/dev/null; then
    userdel anonymous 2>/dev/null
    ACTION_LOG="$ACTION_LOG anonymous 계정을 삭제했습니다."
fi
...
if [ -n "$VSFTPD_CONF" ]; then
    if grep -v "^#" "$VSFTPD_CONF" 2>/dev/null | grep -qiE "anonymous_enable\s*=\s*YES"; then
        ...
        ACTION_LOG="$ACTION_LOG vsftpd anonymous_enable=NO로 설정하고 재시작했습니다."
    fi
fi
...
if [ -n "$PROFTPD_CONF" ]; then
    ...
    if [ -n "$ANON_BLOCK" ]; then
        ...
        ACTION_LOG="$ACTION_LOG proftpd Anonymous 블록을 주석처리하고 재시작했습니다."
    fi
fi
...
if [ -f "/etc/exports" ]; then
    if grep -v "^#" /etc/exports 2>/dev/null | grep -qE "(anonuid|anongid)|\(([^)]*,)?anon([=,)][^)]*)?\)"; then
        cp /etc/exports /etc/exports.bak_$(date +%Y%m%d_%H%M%S)
        # anon, anonuid, anongid 옵션 제거
        sed -Ei 's/([,(])anon([,)=])/\1\2/g' /etc/exports
        sed -i 's/,anonuid=[0-9]*//g' /etc/exports
        sed -i 's/,anongid=[0-9]*//g' /etc/exports
        sed -i 's/anonuid=[0-9]*,//g' /etc/exports
        sed -i 's/anongid=[0-9]*,//g' /etc/exports
        sed -Ei 's/,\s*,/,/g; s/\(,/\(/g; s/,\)/\)/g' /etc/exports
        exportfs -ra 2>/dev/null
        ACTION_LOG="$ACTION_LOG NFS anon/anonuid/anongid 옵션을 제거하고 exportfs -ra를 실행했습니다."
    fi
fi
...
if [ -f "/etc/samba/smb.conf" ]; then
    if grep -v "^#" /etc/samba/smb.conf 2>/dev/null | grep -qiE "guest\s*ok\s*=\s*yes"; then
        ...
        ACTION_LOG="$ACTION_LOG Samba guest ok = no로 설정하고 reload했습니다."
    fi
fi

if [ -n "$ACTION_LOG" ]; then
    ACTION_LOG="공유 서비스(FTP/NFS/Samba)의 익명 접근 설정을 제한했습니다."
else
    ACTION_LOG="익명 접근 설정이 이미 제한된 상태입니다."
fi

STATUS="PASS"
EVIDENCE="공유 서비스에 대한 익명 접근이 제한되어 있습니다."
