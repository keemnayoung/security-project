...
# @Description : FTP root 접속 제한 설정
# @Reference : 2026 KISA 주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드
# ============================================================================

# [보완] U-57 root 계정 FTP 접속 제한
...
block_root() {
    ...
    if [ -f "$file" ]; then
        if ! grep -v "^#" "$file" | grep -q "^root$"; then
            BEFORE_SETTING="$BEFORE_SETTING $file root 미포함;"
            # 주석된 root가 있으면 주석 해제, 없으면 추가
            if grep -q "^#root" "$file"; then
                sed -i 's/^#root/root/' "$file"
                ACTION_LOG="$ACTION_LOG $file #root 주석 해제;"
            else
                echo "root" >> "$file"
                ACTION_LOG="$ACTION_LOG $file root 추가;"
            fi
        fi
    fi
}
...
unallow_root() {
    ...
    if [ -f "$file" ]; then
        if grep -v "^#" "$file" | grep -q "^root$"; then
            BEFORE_SETTING="$BEFORE_SETTING $file(Whitelist) root 포함;"
            sed -i '/^root$/d' "$file"
            ACTION_LOG="$ACTION_LOG $file(Whitelist) root 제거;"
        fi
    fi
}
...
if command -v proftpd &>/dev/null; then
    ...
    if grep -q "UseFtpUsers off" "$CONF" 2>/dev/null; then
         if ! grep -q "RootLogin off" "$CONF"; then
             BEFORE_SETTING="$BEFORE_SETTING ProFTP RootLogin 미설정;"
             echo "RootLogin off" >> "$CONF"
             ACTION_LOG="$ACTION_LOG ProFTP RootLogin off 추가;"
...
AFTER_SETTING="FTP root 접속 제한 조치 완료"
[ -z "$ACTION_LOG" ] && ACTION_LOG="이미 차단되어 있거나 FTP 서비스 없음"
