...
if [ -f "$TARGET_FILE" ]; then
    ...
    if [ $VULNERABLE -eq 1 ]; then
        STATUS="FAIL"
        EVIDENCE="/etc/sudoers 파일의 설정이 미흡하여, 비인가 사용자가 sudo 권한을 조작할 수 있는 위험이 있습니다. "
    else
        STATUS="PASS"
        EVIDENCE="/etc/sudoers 파일이 적절하게 설정되어 있습니다."
    fi
else
    STATUS="PASS"
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "chown root:root /etc/sudoers && chmod 440 /etc/sudoers 명령으로 소유자 및 권한을 설정해야 합니다.",
