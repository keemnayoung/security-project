...
if [ -n "$NFS_SERVICES" ]; then
    VULNERABLE=1
    DETAIL="활성화된 서비스는 ${NFS_SERVICES}입니다."
fi

# 결과 판단
if [ $VULNERABLE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="불필요한 NFS 서비스가 활성화되어 있어 비인가 파일 접근이 발생할 수 있는 위험이 있습니다. ${DETAIL}"
else
    STATUS="PASS"
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "NFS 서비스가 불필요한 경우 systemctl stop nfs-server 및 systemctl disable nfs-server로 비활성화해야 합니다.",
