    ...
    for conf in "${CONF_FILES[@]}"; do
        if [ -f "$conf" ]; then
            ...
            if [ -n "$V3_USER_CHECK" ]; then
                EVIDENCE="$EVIDENCE $conf: SNMPv3 사용자 설정이 발견되었습니다."
                ...
                if [ -n "$COMMUNITY_CHECK" ]; then
                    if echo "$COMMUNITY_CHECK" | grep -qE "public|private"; then
                        VULNERABLE=1
                        EVIDENCE="$EVIDENCE SNMPv3 설정은 있으나, 취약한 Community String(public/private)이 활성화되어 있습니다."
                    fi
                else
                    STATUS="PASS"
                    EVIDENCE="$EVIDENCE SNMPv3 사용 중이며 Community String 설정이 없어 안전합니다."
                fi
            else
                # V3 설정이 없음
                VULNERABLE=1
                EVIDENCE="$EVIDENCE $conf: SNMPv3 사용자 설정이 없어 v1/v2c 사용이 추정됩니다."
                
                if [ -n "$COMMUNITY_CHECK" ]; then
                     EVIDENCE="$EVIDENCE Community String 설정이 존재합니다($COMMUNITY_CHECK)."
                fi
            fi
        fi
    done
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "SNMPv3 사용을 권장하며, SNMPv1/v2c 사용 시 community string을 복잡하게 변경해야 합니다.",
