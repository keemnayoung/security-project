...
LIMIT_EOF
                ...
                if ! grep -A5 "<Limit LOGIN>" "$CONF" | grep -qi "DenyUser.*root"; then
                    ...
                    ACTION_LOG="$ACTION_LOG <Limit LOGIN> 존재하나 DenyUser root 수동 확인이 필요합니다;"
                fi
            fi
...
if [ -n "$ACTION_LOG" ]; then
    ACTION_LOG="FTP 서비스 접근 제어 설정을 수정하여 비인가자의 접근을 제한했습니다."
else
    ACTION_LOG="대상 파일이 없거나 이미 적절하게 설정되어 있습니다."
fi
