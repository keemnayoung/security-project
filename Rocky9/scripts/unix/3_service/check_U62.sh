...
for file in "/etc/motd" "/etc/issue"; do
    if [ -f "$file" ]; then
        ...
        if [ -z "$CONTENT" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE $file 경고 메시지가 없습니다."
        else
            EVIDENCE="$EVIDENCE $file 경고 메시지가 존재합니다."
        fi
    fi
done
...
if [ -f "/etc/issue.net" ]; then
    ...
    if [ -z "$CONTENT" ]; then
        VULNERABLE=1
        EVIDENCE="$EVIDENCE /etc/issue.net 경고 메시지가 없습니다."
    else
        EVIDENCE="$EVIDENCE /etc/issue.net 경고 메시지가 존재합니다."
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q sshd; then
    ...
    if [ -f "$SSH_CONF" ]; then
        ...
        if [ -z "$BANNER" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE SSH Banner 설정이 없습니다."
        else
            EVIDENCE="$EVIDENCE SSH Banner: $BANNER."
        fi
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q sendmail; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$GREETING" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE Sendmail SmtpGreetingMessage가 없습니다."
        fi
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q postfix; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$BANNER" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE Postfix smtpd_banner가 없습니다."
        fi
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q vsftpd; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$BANNER" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE vsFTPd ftpd_banner가 없습니다."
        fi
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q proftpd; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$DISPLAY" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE ProFTP DisplayLogin이 없습니다."
        else
...
if systemctl list-units --type=service 2>/dev/null | grep -q exim; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$BANNER" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE Exim smtp_banner가 없습니다."
        fi
    fi
fi
...
if systemctl list-units --type=service 2>/dev/null | grep -q named; then
    ...
    if [ -f "$CONF" ]; then
        ...
        if [ -z "$VERSION" ]; then
            VULNERABLE=1
            EVIDENCE="$EVIDENCE DNS version 설정이 없습니다."
        fi
    fi
fi
...
if [ $VULNERABLE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="로그온 경고 메시지가 설정되어 있지 않아, 불법 접속에 대한 경고가 표시되지 않습니다. $EVIDENCE"
else
    STATUS="PASS"
    EVIDENCE="로그온 경고 메시지가 설정되어 있습니다."
fi


IMPACT_LEVEL="LOW"
ACTION_IMPACT="이 조치를 적용하더라도 일반적인 시스템 운영에는 영향이 없으나, 로그인/접속 배너 문구가 정책 문구로 변경되므로 운영 표준 문구(법적 고지/보안 경고)를 사전에 합의한 뒤 서비스별 설정 파일에 일관되게 반영해야 합니다."
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "/etc/issue, /etc/motd에 경고 메시지를 설정하고, sshd_config에 Banner /etc/issue.net을 설정한 후 서비스를 재시작해야 합니다.",
