...
if [ $VULNERABLE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="DoS 공격에 취약한 서비스(echo, discard, daytime, chargen)가 활성화되어 있어, 서비스 거부 공격에 악용될 수 있는 위험이 있습니다. $EVIDENCE"
else
    STATUS="PASS"
    EVIDENCE="DoS 공격에 취약한 서비스(echo,discard,daytime,chargen)가 비활성화되어 있습니다."
fi
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "xinetd에서 echo, discard, daytime, chargen 서비스를 disable=yes로 설정하고 xinetd를 재시작해야 합니다.",
