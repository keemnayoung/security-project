...
if [ -f "/etc/inetd.conf" ]; then
    ...
    if grep -v "^#" "$TARGET_FILE" 2>/dev/null | grep -qE "^[[:space:]]*rpc"; then
        ...
        EVIDENCE="$EVIDENCE /etc/inetd.conf에 RPC 서비스 활성화: $RPC_INETD;"
    fi
fi
        ...
        if [ -f "$conf" ]; then
            ...
            if echo "$conf" | grep -qi "rpc" || grep -q "service.*rpc" "$conf" 2>/dev/null; then
                if grep -qiE "disable\s*=\s*no" "$conf" 2>/dev/null; then
                    VULNERABLE=1
                    EVIDENCE="$EVIDENCE $(basename $conf)에서 disable=no;"
                fi
            fi
        fi
    done
...
if [ -n "$RPC_SERVICES" ]; then
    VULNERABLE=1
    EVIDENCE="$EVIDENCE systemd RPC 서비스 활성화: $RPC_SERVICES;"
fi

# 결과 판단
if [ $VULNERABLE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="불필요한 RPC 서비스 활성화:$EVIDENCE"
else
    STATUS="PASS"
    EVIDENCE="불필요한 RPC 서비스가 비활성화됨"
fi
