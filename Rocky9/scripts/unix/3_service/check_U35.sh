...
if grep -q "^ftp:" /etc/passwd 2>/dev/null; then
    VULNERABLE=1
    SERVICE_EXISTS=1
    EVIDENCE="$EVIDENCE /etc/passwd에 ftp 계정 존재;"
fi
if grep -q "^anonymous:" /etc/passwd 2>/dev/null; then
    VULNERABLE=1
    SERVICE_EXISTS=1
    EVIDENCE="$EVIDENCE /etc/passwd에 anonymous 계정 존재;"
fi
...
if [ -n "$VSFTPD_CONF" ]; then
    ...
    if grep -v "^#" "$VSFTPD_CONF" 2>/dev/null | grep -qiE "anonymous_enable\s*=\s*YES"; then
        VULNERABLE=1
        EVIDENCE="$EVIDENCE $VSFTPD_CONF에서 anonymous_enable=YES;"
    fi
fi
...
if [ -n "$PROFTPD_CONF" ]; then
    ...
    if [ -n "$ANON_BLOCK" ]; then
        VULNERABLE=1
        EVIDENCE="$EVIDENCE $PROFTPD_CONF에 Anonymous 블록 활성화;"
    fi
fi
...
if [ -f "/etc/exports" ]; then
    ...
    if grep -v "^#" /etc/exports 2>/dev/null | grep -qE "(anonuid|anongid)"; then
        VULNERABLE=1
        EVIDENCE="$EVIDENCE /etc/exports에 anonuid/anongid 설정;"
    fi
fi
...
if [ -f "/etc/samba/smb.conf" ]; then
    ...
    if grep -v "^#" /etc/samba/smb.conf 2>/dev/null | grep -qiE "guest\s*ok\s*=\s*yes"; then
        VULNERABLE=1
        EVIDENCE="$EVIDENCE /etc/samba/smb.conf에서 guest ok = yes;"
    fi
fi
...
if [ $SERVICE_EXISTS -eq 0 ]; then
    STATUS="PASS"
    EVIDENCE="공유 서비스(FTP/NFS/Samba) 미사용 (양호)"
elif [ $VULNERABLE -eq 1 ]; then
    STATUS="FAIL"
    EVIDENCE="공유 서비스 익명 접근 허용:$EVIDENCE"
else
    STATUS="PASS"
    EVIDENCE="공유 서비스에 대해 익명 접근이 제한됨"
fi
...
cat << EOF
{
    ...
    "status": "$STATUS",
    "evidence": "$EVIDENCE",
    "guide": "vsFTPd: anonymous_enable=NO 설정, Samba: guest ok=no 설정, NFS: /etc/exports에서 insecure 옵션 제거 후 서비스 재시작하세요.",
