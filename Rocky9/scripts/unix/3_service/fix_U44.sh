...
if [ -f "/etc/inetd.conf" ]; then
    ...
    if [ $INETD_MODIFIED -eq 1 ]; then
        ...
        ACTION_LOG="$ACTION_LOG /etc/inetd.conf tftp/talk 서비스를 주석 처리하고 inetd를 재시작했습니다."
    fi
fi
...
if [ -n "$ACTION_LOG" ]; then
    ACTION_LOG="불필요한 tftp, talk, ntalk 서비스를 비활성화했습니다."
else
    ACTION_LOG="tftp, talk, ntalk 서비스가 이미 비활성화되어 있어 추가 조치 없이 쒑호한 상태를 유지합니다."
fi
