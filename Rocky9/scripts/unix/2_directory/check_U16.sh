...
if [ ! -f "$TARGET_FILE" ]; then
    ...
    if [ "$FILE_OWNER" = "root" ] && [ "$FILE_PERM" -le 644 ]; then
        STATUS="PASS"
        REASON_LINE="/etc/passwd 파일의 소유자가 root이고 권한이 $FILE_PERM(644 이하)로 설정되어 있으므로 이 항목에 대한 보안 위협이 없습니다."
    else
        STATUS="FAIL"
        REASON_LINE="/etc/passwd 파일의 소유자가 $FILE_OWNER 이거나 권한이 $FILE_PERM(644 초과)로 설정되어 있어 비인가 사용자가 파일을 변경하거나 과도하게 접근할 위험이 있으므로 취약합니다. 소유자를 root로 변경하고 권한을 644 이하로 설정해야 합니다."
    fi
