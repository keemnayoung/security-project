...
CHECK_COMMAND='stat -c "%U %a %n" /etc/inetd.conf /etc/xinetd.conf 2>/dev/null; find /etc/systemd -type f -print0 2>/dev/null | xargs -0 -I{} stat -c "%U %a %n" "{}" 2>/dev/null'
...
if [ "$STATUS" = "PASS" ]; then
    REASON_LINE="/etc/inetd.conf, /etc/xinetd.conf 및 /etc/systemd 내 파일의 소유자가 root이고 권한이 600 이하로 설정되어 있어 비인가 사용자의 임의 수정이 제한되므로 이 항목에 대한 보안 위협이 없습니다."
    ...
    REASON_LINE="/etc/inetd.conf, /etc/xinetd.conf 또는 /etc/systemd 내 일부 파일의 소유자가 root가 아니거나 권한이 600 초과로 설정되어 비인가 사용자가 설정을 변경할 위험이 있으므로 취약합니다. 해당 파일들의 소유자를 root로 변경하고 권한을 600 이하로 설정해야 합니다."
