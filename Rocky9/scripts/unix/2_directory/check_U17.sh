...
# 점검 대상 파일 목록 수집 (systemd 방식: 상위 + 하위 디렉터리 1레벨 포함)
SYSTEMD_FILES=""
if [ -d /etc/systemd/system ]; then
    SYSTEMD_FILES=$( (readlink -f /etc/systemd/system/* 2>/dev/null; readlink -f /etc/systemd/system/*/* 2>/dev/null) )
fi
...
if [ -z "$ALL_FILES" ]; then
        ...
        if [ -d "$FILE" ]; then
            for SUB in "$FILE"/*; do
                ...
                if [ "$OWNER" != "root" ] || [ "$OTHERS_WRITE" = "w" ]; then
                    FOUND_VULN="Y"
                    STATUS="FAIL"
                    VULN_LINES+="$SUB owner=$OWNER perm=$PERM"$'\n'
                fi
            done
            continue
        fi
    ...
    if [ "$FOUND_VULN" = "Y" ]; then
        REASON_LINE="시스템 시작 스크립트 파일의 소유자가 root가 아니거나 others 쓰기 권한이 허용되어 있어 임의 수정 및 권한 상승 위험이 있으므로 취약합니다. 소유자를 root로 변경하고 others 쓰기 권한(o-w)을 제거해야 합니다."
